	...
	public function Registration()
			{
				...
				$data=(object)array(
								...
								"ontact"=>$this->input->post("Contact"),
								"username"=>$this->input->post("username"),
...
$m=urlencode($m);    
 sendSms($mobile_number,$m,$country);
...
if($value)
	{
      ...
      $message=$this->load->view('emails/welcome_emails',$emaildata,TRUE);
				       $this->email->set_newline("\r\n");
				       $this->email->from('customers@ip-ae.com','Ip-AE');
				       $this->email->to( $emaildata['username']);
				       $this->email->subject($subject);
				       $this->email->message($message);
				       $this->email->send();
...
public function submit_support_form(){
		...
		$data['datetime']=date('Y-m-d h:i:s');
		 ...
		 $this->form_validation->set_rules('order_id', 'Invoice No', 'required');
...
public function getcity($id)
 {  
	 ...
	 foreach($result as $each){ 
        echo "<option value='".$each->id."'>'".$each->cityname."'</option>"; 
  	}  
...
{
	$result=$this->Ipae_model->getReportLastHours();
	$emaildata['total_no_order']=$result['total_no_order'];
	$emaildata['cod']=$result['cod'];
	$emaildata['card']=$result['card'];
	$emaildata['placed']=$result['placed'];
	$emaildata['in_process']=$result['in_process'];
	$emaildata['cancelled_customer']=$result['cancelled_customer'];
	$emaildata['cancelled_ipae']=$result['cancelled_ipae'];
	$emaildata['shipped']=$result['shipped'];
	$emaildata['delivered']=$result['delivered'];
	$emaildata['product_detail']=$result['product_detail'];
	$message=$this->load->view('emails/last_two_hours_report',$emaildata,TRUE);
 
	 
   	$subject = "Daily Report iP-AE";
     $this->email->set_newline("\r\n");
    $this->email->from('customers@ip-ae.com','iP-AE');
    $this->email->to('info@ip-ae.com');
    $this->email->subject($subject);
...
{
	$result=$this->Ipae_model->getDaliyReport();
	$emaildata['total_no_order']=$result['total_no_order'];
	$emaildata['cod']=$result['cod'];
	$emaildata['card']=$result['card'];
	$emaildata['placed']=$result['placed'];
	...
	$emaildata['product_detail']=$result['product_detail'];
			...
			$message=$this->load->view('emails/day_report',$emaildata,TRUE);
