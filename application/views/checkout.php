 ...
 ?>
  ...
  <div class="row">
    ...
    <?php foreach($data as $each){ ?>
        <option value="<?php echo $each->zone_id; ?>" <?php if($address_details['emirates']==$each->zone_id){ echo 'selected';}?>><?php echo $each->state_name; ?></option>';
    <?php } ?>
</select>
          ...
          <select class="form-control" id="updatebyajax" name="area">
		      <?php
			    if($address_details['area']!=''){
				 $result=$this->Ipae_model->getAllcity($address_details['emirates']);
	 foreach($result as $each){ ?>
        <option value="<?php echo $each->id; ?>" <?php if($address_details['area']==$each->id){ echo 'selected';}?> ><?php echo $each->cityname; ?></option> 
    <?php	
				}
                }else{  
			  ?>
      ...
      <div class="row">
        <div class="form-group col-sm-12">
          ...
          <textarea id="streetname" name="streetname" rows="1" class="form-control" placeholder="Street Name/No." ><?php echo $address_details['street'];?></textarea>
		  <span class="error5"><span>
        </div>
        <div class="form-group col-sm-12">
          <label for="postal-code">Building Name/No.</label>
          <input class="form-control" id="buildingname" name="buildingname" placeholder="Building Name/No." value="<?php echo $address_details['building'];?>" type="text">
               ...
               <td class="text-right">AED <?php echo $subtotal; ?>  
                                    ...
                                    <div id="checkout-review-submit">
                                       <div class="cart-btn-3" id="review-buttons-container">
										 ...
										 <?php if($outStock>0){?>
						<button type="button" title="Place Order" class="btn btn-default" id="submitform"><span>Place Order</span></button>	
					    <?php }else{?>
...
var	id=$('#emirates').val();
  ...
  var outStock=$('#outStock').val();
	if(outStock>0){
	$(".error_stock").html('<span class=error_stock style="color:red">&nbsp;&nbsp;&nbsp;Some itemes are out of stock, please remove item.</span>');
    return false;  	
	}
...
if($.trim(firstname)==''){
 $(".error1").html('<span class=error>Please Enter First Name.<error>');
   $("#firstname").focus();
   return false;   
}
else{$(".error1").html('');}
...
if($.trim(lastname)==''){
  $(".error2").html('<span class=error>Please Enter Last Name.<error>');
   $("#lastname").focus();
   return false;   
}
else{$(".error2").html('');}
...
if($.trim(emirates)==''){
   $(".error3").html('<span class=error>Please Select Emirates.<error>');
   $("#emirates").focus();
   return false;   
}
else{$(".error3").html('');}
...
if($.trim(updatebyajax)==''){
    $(".error4").html('<span class=error>Please Select Area.<error>');
   $("#updatebyajax").focus();
   return false;   
}
else{$(".error4").html('');}
...
if($.trim(streetname)==''){
   $(".error5").html('<span class=error>Please Enter Street Name/No.<error>');
   $("#streetname").focus();
   return false;   
}
else{$(".error5").html('');}
...
if($.trim(buildingname)==''){
 $(".error6").html('<span class=error>Please Enter Building Name/No.<error>');
   $("#buildingname").focus();
   return false;   
}
else{$(".error6").html('');}
